...
# Big thanks to everyone at Udacity for making this code possible!

app = Flask(__name__)

CLIENT_ID = json.loads(
                open('client_secrets.json', 'r').read())['web']['client_id']
APPLICATION_NAME = "Category Project"


engine = create_engine(
                'sqlite:///categoryproject.db',
...
def gconnect():
    ...
    # see if the user exists, if they don't make a new one
...
# Revoke the user's token and reset login_session
...
def gdisconnect():
    ...
    if result['status'] == '200':
        ...
        # if the given token was invalid, somehow
        response = make_response(
            json.dumps('Failed to revoke token for given user.', 400))
        response.headers['Content-Type'] = 'application/json'
        return response

# JSON APIs to allow for data viewing
...
def ItemJSON(category_id, item_id):
    An_Item = session.query(Item).filter_by(id=item_id).one()
    return jsonify(An_Item=An_Item.serialize)
